-- Kanal bayar rinci — tunai, QRIS, transfer, kartu debit/kredit (`card`).
--
-- Sampai migrasi ini `orders.payment_method` (enum `payment_method`, 0001_init)
-- hanya tahu 'cash'/'non_cash'. Itu sudah cukup untuk memutuskan "masuk laci
-- atau tidak" (dipakai app/pay.tsx dan mobile/db/shift.ts), tapi tidak cukup
-- untuk merinci penjualan per kanal di struk Tutup Kasir — semua yang bukan
-- tunai jatuh ke satu ember "Non Tunai". TO_DO.md sudah mencatat kekurangan
-- ini di bawah judul "Kanal bayar berhenti di Tunai/Non Tunai".
--
-- KOLOM BARU, BUKAN PELEBARAN ENUM `payment_method`. Postgres sendiri bisa
-- memperlebar enum dengan aman (`alter type ... add value`), tapi perangkat
-- lokal (mobile/db/migrations.ts) tidak bisa — SQLite tidak punya cara
-- melonggarkan CHECK tanpa membangun ulang tabel `orders`, operasi yang sudah
-- sekali ditolak (lihat komentar V2) karena risikonya terhadap histori order
-- di perangkat yang sudah beredar. Supaya kedua sisi (lokal dan server) punya
-- bentuk yang sama persis, server juga memakai kolom baru text+CHECK, BUKAN
-- memperlebar enum lama. `payment_method` tetap satu-satunya sumber
-- "masuk laci atau tidak" dan tidak disentuh sama sekali di sini.
--
-- BACKFILL: setiap order lunas SUDAH PASTI punya payment_method
-- (constraint paid_fields_consistent sejak 0001_init) — bukan kasus "order
-- tanpa metode". Yang di-backfill adalah PEMETAAN ke kanal yang lebih rinci:
-- 'cash' -> 'cash' apa adanya, itu bukan tebakan. 'non_cash' TIDAK dipetakan
-- ke 'transfer': layar Pelunasan lama memang hanya punya tab "Transfer" untuk
-- non tunai, tapi kanal SEBENARNYA tiap transaksi lama (QRIS lewat aplikasi
-- pihak ketiga yang dicatat kasir sebagai "Transfer", transfer bank sungguhan,
-- dll) tidak pernah tersimpan di baris manapun — memetakannya ke 'transfer'
-- adalah TEBAKAN yang permanen begitu berjalan di hosted, karena datanya tidak
-- bisa dipulihkan sesudahnya. 'non_cash' -> 'non_cash_legacy': nilai khusus
-- data historis, dibedakan dari kanal yang benar-benar diketahui.
--
-- DUA TEMPAT LAIN HARUS IKUT BERUBAH:
--   1. push_order — menerima dan menyimpan payment_channel dari payload.
--   2. laporan_penjualan_harian (0027) — kolom non_tunai-nya memfilter
--      `payment_method = 'non_cash'` secara harfiah. Begitu ada order ber-
--      payment_method 'non_cash' yang lahir dari kanal baru... sebetulnya
--      TIDAK: payment_method tetap 'non_cash' untuk qris/transfer/card,
--      hanya payment_channel yang rinci. Jadi laporan dashboard yang
--      menyaring `payment_method` TIDAK perlu diubah — ia tetap benar tanpa
--      disentuh. Baris ini sengaja dicatat supaya tidak ada yang "memperbaiki"
--      laporan itu berdasarkan asumsi yang keliru tentang bagaimana kolom ini
--      bekerja.
...
-- 'non_cash_legacy' TIDAK ADA di layar Pelunasan (mobile/app/pay.tsx) dan
-- TIDAK ADA di PaymentChannel (mobile/lib/types.ts) — tidak ada jalur di
-- aplikasi yang bisa mengirim nilai ini untuk transaksi baru. Ia ada di
-- CHECK ini semata-mata supaya backfill di bawah punya tempat menaruh kanal
-- non tunai lama yang sungguhan tidak diketahui, tanpa berbohong bahwa itu
-- 'transfer'. Kalau nilai ini pernah terlihat di payload push_order, itu
-- bug di klien, bukan penggunaan yang sah.
alter table orders
  add column if not exists payment_channel text
  check (payment_channel is null or payment_channel in
    ('cash', 'qris', 'transfer', 'card', 'non_cash_legacy'));

-- `and payment_channel is null` membuat baris ini aman dijalankan dua kali:
-- jalan pertama mengisi setiap order lunas, jalan kedua tidak menyentuh
-- apa pun karena tidak ada lagi baris yang kosong.
update orders
...
-- ============================================================ push_order
--
-- Salinan utuh dari 0026. Satu-satunya perubahan: `v_channel` dibaca dari
-- payload dan disisipkan/diperbarui bersama payment_method. Tidak dicast ke
-- tipe enum apa pun — kolomnya text+CHECK, jadi nilai di luar daftar yang
-- sah ditolak oleh constraint tabel, sama seperti kolom text+CHECK lain di
-- skema ini (shifts.label, cash_movements.direction/method).
--
-- APK LAMA yang belum mengirim payment_channel: `nullif(..., '')` jatuh ke
-- null, yang sah menurut CHECK-nya. Order dari APK lama tetap tersimpan,
-- hanya tanpa rincian kanal — sama seperti order dari APK yang lebih lama
-- lagi yang tidak pernah mengirim payment_method sama sekali dulu.
...
declare
  ...
  v_channel    text   := nullif(p_order ->> 'payment_channel', '');
